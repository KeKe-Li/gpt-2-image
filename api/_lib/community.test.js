...
import { communityStatusPayload } from '../community/status.js';
...
test('community status payload carries the server-owned terms version', () => {
  const payload = communityStatusPayload(
    { termsVersion: '2026-07-22', priceLabel: '¥9.90' },
    { authenticated: false, eligible: false }
  );
  assert.equal(payload.termsVersion, '2026-07-22');
  assert.equal(payload.priceLabel, '¥9.90');
